...
from rlcard.games.bluffgame import Game
...
class BluffGameEnv(Env):
    ...
    def __init__(self, config):
        ...
        self.state_shape = [[76] for _ in range(self.num_players)] #? what is 76 --> observation size
        self.action_shape = [None for _ in range(self.num_players)]

        with open(os.path.join(rlcard.__path__[0], 'games/bluffgame/card2index.json'), 'r') as file:
    ...
    def _extract_state(self, state):
        ...
        hand = state['hand']
        obs = np.zeros(76) # Changed all obs
        obs[self.card2index[hand]] = 1
        obs[state['my_chips']+10] = 1
        obs[sum(state['all_chips'])-state['my_chips']+43] = 1 # update obs skip
