    ...
    with MultiLogger(args.log_dir, agent_keys) as multi_logger:
        while total_episodes_trained < args.num_episodes:
            ...
            print(
                ...
                f"{min(total_episodes_trained + switch_frequency, args.num_episodes)} ---"
            )
            ...
            while episodes_in_chunk < switch_frequency and total_episodes_trained < args.num_episodes:
                ...
                if total_episodes_trained > 0 and total_episodes_trained % args.evaluate_every == 0:
                    ...
                    eval_env = rlcard.make(
                        args.env, 
                        config={'seed': args.seed + total_episodes_trained + 10000}
                    )

                    # === Evaluation vs Random ===
                    print("  --- vs RandomAgent ---")
                    eval_opponents_random = [
                        RandomAgent(num_actions=eval_env.num_actions) 
                        for _ in range(1, eval_env.num_players)
                    ]
                    # DQN vs Random
                    ...
                    eval_opponents_aggressive = [
                        AggressiveAgent(num_actions=eval_env.num_actions) 
                        for _ in range(1, eval_env.num_players)
                    ]
                    # DQN vs Aggressive
                    eval_agents_dqn_agg = [agent_dqn] + eval_opponents_aggressive
                    eval_env.set_agents(eval_agents_dqn_agg)
                    ...
                    eval_agents_vaedqn_agg = [agent_vaedqn] + eval_opponents_aggressive
                    eval_env.set_agents(eval_agents_vaedqn_agg)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_dir', type=str, default='experiments/bluffgame_dqn_vs_vaedqn_mutual_eval_v5444/') # Changed default dir
