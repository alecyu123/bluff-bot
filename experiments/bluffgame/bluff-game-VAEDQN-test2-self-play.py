...
import os # Keep os import for log_dir handling and CUDA_VISIBLE_DEVICES
import argparse
...
# Original train function provided by user (kept for reference/potential use)
def train(args):
    ...
    # Save the agent object directly (as in the original script)
    # This relies on the agent object being pickleable.
    # If using VAEDQNAgent with custom checkpointing, that logic might be preferred.
    try:
        ...
        # Example: if hasattr(agent, 'save_checkpoint'): agent.save_checkpoint(...)


# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# +++ Modified Multi-Train Function (Alternating, Separate Logging) +++
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
def multi_train(args):
    ''' Train DQN and VAEDQN agents, alternating training.
        Evaluates BOTH agents against RandomAgent at each interval.
        Logs performance to SEPARATE files.
    '''
    ...
    agent_names = ["DQN", "VAE_DQN"]

    # --- Initialize SEPARATE Loggers ---
    os.makedirs(args.log_dir, exist_ok=True)
    log_dir_dqn = os.path.join(args.log_dir, 'dqn_log')
    log_dir_vaedqn = os.path.join(args.log_dir, 'vaedqn_log')
    os.makedirs(log_dir_dqn, exist_ok=True)
    os.makedirs(log_dir_vaedqn, exist_ok=True)

    # Instantiate loggers here, but get paths later
    logger_dqn = Logger(log_dir_dqn)
    logger_vaedqn = Logger(log_dir_vaedqn)

    # Training Loop Setup
    total_episodes_trained = 0
    current_learner_idx = 0

    # Use try/finally to ensure loggers are closed
    try:
        while total_episodes_trained < args.num_episodes:
            ...
            current_agent_name = agent_names[current_learner_idx]
            ...
            if opponent_agent == other_learner_agent: opponent_name = agent_names[1 - current_learner_idx]

            print(f"\n--- Episodes {total_episodes_trained+1} -> {min(total_episodes_trained + switch_frequency, args.num_episodes)} ---")
            print(f"--- Training: {current_agent_name} --- Opponent: {opponent_name} ---")
            ...
            while episodes_in_chunk < switch_frequency and total_episodes_trained < args.num_episodes:
                ...
                # Check if it's time to evaluate (do it right before switching if switch_frequency aligns)
                # Evaluate at the *end* of the chunk or at the overall evaluation frequency step
                time_to_evaluate = (episodes_in_chunk == switch_frequency - 1) or \
                                   (total_episodes_trained > 0 and total_episodes_trained % args.evaluate_every == 0)

                if time_to_evaluate:
                    print(f"\n--- Evaluating ALL agents at episode {total_episodes_trained} ---") # Log with current total
                    eval_env = rlcard.make(args.env, config={'seed': args.seed + total_episodes_trained + 10000})
                    eval_opponents = [RandomAgent(num_actions=eval_env.num_actions) for _ in range(1, eval_env.num_players)]

                    # Evaluate DQN Agent
                    eval_agents_dqn = [agent_dqn] + eval_opponents
                    eval_env.set_agents(eval_agents_dqn)
                    perf_dqn = tournament(eval_env, args.num_eval_games)[0]
                    # Use the logger instance directly to log
                    logger_dqn.log(f"episode,{total_episodes_trained}\n") # Log episode manually first
                    logger_dqn.log_performance(total_episodes_trained, perf_dqn) # Log performance value
                    print(f"  Evaluation result (DQN vs Random): {perf_dqn}")

                    # Evaluate VAEDQN Agent
                    eval_agents_vaedqn = [agent_vaedqn] + eval_opponents
                    eval_env.set_agents(eval_agents_vaedqn)
                    perf_vaedqn = tournament(eval_env, args.num_eval_games)[0]
                    # Use the logger instance directly to log
                    logger_vaedqn.log(f"episode,{total_episodes_trained}\n") # Log episode manually first
                    logger_vaedqn.log_performance(total_episodes_trained, perf_vaedqn) # Log performance value
                    print(f"  Evaluation result (VAE_DQN vs Random): {perf_vaedqn}")
                    print("-------------------------------------")
        ...
        # --- Retrieve logger paths AFTER loop ---
        dqn_csv_path, dqn_fig_path = logger_dqn.csv_path, logger_dqn.fig_path
        vaedqn_csv_path, vaedqn_fig_path = logger_vaedqn.csv_path, logger_vaedqn.fig_path

        # Ensure loggers are closed properly
        logger_dqn.close()
        logger_vaedqn.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_dir', type=str, default='experiments/bluffgame_dqn_vs_vaedqn_result_v3/') # Changed default dir
